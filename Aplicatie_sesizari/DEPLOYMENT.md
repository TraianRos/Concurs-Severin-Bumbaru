...
Pentru email si Web Push in demo:
...
- utilizator `user`
- proiect in `/home/user/Documents/Aplicatie sesizari`
- bind pe `127.0.0.1:5000`

### 2. Creeaza fisierul `.env`

```bash
cd "/home/user/Documents/Aplicatie sesizari"
cp .env.example .env
```
...
Pentru notificari email/Web Push:
...
```bash
sudo cp "/home/user/Documents/Aplicatie_sesizari/deploy/aplicatie-sesizari.service" /etc/systemd/system/aplicatie-sesizari.service
...
```bash
sudo cp "/home/user/Documents/Aplicatie_sesizari/deploy/cloudflared-quick-tunnel.service" /etc/systemd/system/cloudflared-quick-tunnel.service
sudo systemctl daemon-reload
...
## Configurare pentru email si Web Push

Adauga in `.env` valorile reale pentru SMTP si cheile VAPID:
...
SMTP_FROM_EMAIL=demo@exemplu.ro
SMTP_USE_TLS=true
SMTP_USE_SSL=false
EMAIL_SUBJECT_PREFIX=Orasul in AlertA
WEB_PUSH_PUBLIC_KEY=...
WEB_PUSH_PRIVATE_KEY=...
WEB_PUSH_SUBJECT=mailto:demo@exemplu.ro
```

Observatii practice:

- emailul este trimis sincron, best-effort, in acelasi request
- Web Push functioneaza in browser doar pe `https://` sau `localhost`
- daca folosesti `cloudflared`, URL-ul public `https://*.trycloudflare.com` este suficient pentru demo Web Push
