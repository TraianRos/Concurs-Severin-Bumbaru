...
class AuthService:
    ...
    def register_citizen(self, payload: dict) -> dict:
        ...
        if len(full_name) > self._limit("full_name"):
            return {"ok": False, "message": "Numele complet nu poate depăși 120 de caractere."}
        if len(email) > self._limit("email"):
            return {"ok": False, "message": "Adresa de email nu poate depăși 255 de caractere."}
        if len(password) > self._limit("password") or len(confirm_password) > self._limit("password"):
            return {"ok": False, "message": "Parola nu poate depăși 128 de caractere."}
        if "@" not in email:
    ...
    def authenticate(self, email: str, password: str) -> dict:
        ...
        if len(normalized_email) > self._limit("email") or len(password) > self._limit("password"):
            return {"ok": False, "message": "Datele de autentificare depășesc limitele permise."}
