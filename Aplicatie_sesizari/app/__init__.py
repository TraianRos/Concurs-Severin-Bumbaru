    ...
    with db.engine.connect() as connection:
        ...
        connection.exec_driver_sql(
            ...
            CREATE TABLE incidents (
                ...
                visibility_state VARCHAR(32) NOT NULL DEFAULT 'visible',
                created_at DATETIME NOT NULL,
...
def _rebuild_incident_reports_table_with_unique_reporters() -> None:
    with db.engine.connect() as connection:
        connection.exec_driver_sql("PRAGMA foreign_keys=OFF")
        connection.exec_driver_sql("ALTER TABLE incident_reports RENAME TO incident_reports_legacy")
        connection.exec_driver_sql(
            """
            CREATE TABLE incident_reports (
                ...
                CONSTRAINT uq_incident_reporter UNIQUE (incident_id, reporter_id),
        ...
        connection.exec_driver_sql(
            ...
            SELECT
                ...
                report.created_at,
                report.reviewed_at
            FROM incident_reports_legacy AS report
            INNER JOIN (
                SELECT MAX(id) AS kept_id
                FROM incident_reports_legacy
                GROUP BY incident_id, reporter_id
            ) AS deduped ON deduped.kept_id = report.id
            """
        )
        connection.exec_driver_sql("DROP TABLE incident_reports_legacy")
        connection.exec_driver_sql("PRAGMA foreign_keys=ON")
        connection.commit()
...
def ensure_schema_compatibility() -> None:
    if db.engine.dialect.name != "sqlite":
        ...
        if inspector.has_table("incidents"):
            ...
            if "visibility_state" not in incident_columns:
                db.session.execute(
                    text("ALTER TABLE incidents ADD COLUMN visibility_state VARCHAR(32) NOT NULL DEFAULT 'visible'")
                )
    ...
    if inspector.has_table("incident_reports"):
        unique_constraints = {
            constraint.get("name")
            for constraint in inspector.get_unique_constraints("incident_reports")
        }
        if "uq_incident_reporter" not in unique_constraints:
            _rebuild_incident_reports_table_with_unique_reporters()
...
def create_app(config_overrides: dict | None = None) -> Flask:
    ...
    def inject_globals():
        ...
        return {
            ...
            "form_limits": app.config.get("FORM_LIMITS", {}),
